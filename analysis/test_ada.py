...
def compare(
    ...
    assert len(df) == size_group, (
        "The csv file does not contain the right number of scores. If must constain `size_group` scores. Either change the argument `size_group` or give a csv file with {} scores".format(
            size_group
        )
    )
    ...
    if os.path.isfile(path_lf):
        ...
        if len(Z[0]) > comparator.K * n_fits_per_group:
            raise ValueError(
                "Error: you tried to use more group than what was initially declared, this is not allowed by the theory."
            )
        assert "continue" in list(comparator.decisions.values()), (
            "Test finished at last iteration."
        )

    else:
        comparator = MultipleAgentsComparator(
            n=n_fits_per_group,
            K=n_groups,
            B=n_permutations,
...
compare(
    ctx=None,
    input_file="test_csv_domains_1.csv",
    n_groups=4,
    size_group=4,
    n_permutations=0,
    alpha=0.01,
    beta=0,
    comparisons=[(0, 3), (1, 4), (2, 5), (8, 6), (9, 7)],
    seed=None,
    compare_to_first=False,
)
compare(
    ctx=None,
    input_file="test_csv_domains_2.csv",
    n_groups=4,
    size_group=4,
    n_permutations=0,
    alpha=0.01,
...
compare(
    ctx=None,
    input_file="test_csv_domains_3.csv",
    n_groups=4,
    size_group=4,
    n_permutations=0,
    alpha=0.01,
...
compare(
    ctx=None,
    input_file="test_csv_domains_4.csv",
    n_groups=4,
    size_group=4,
    n_permutations=0,
    alpha=0.01,
