...
### PERF with {{ core_value }} CPU * {{ cpu_ops_per_core }} OPS
    ${SUDO_CMD}bash -c "perf stat -a -o /tmp/perf_alone_{{ core_value }}_{{ cpu_ops_per_core }}_$i {% for perf_event in perf_events.iter() %}-e {{ perf_event }} {% endfor %} & echo \$!" > /tmp/perf_pid_$i
