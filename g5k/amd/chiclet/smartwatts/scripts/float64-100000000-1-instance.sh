...
mkdir result-float64-100000000-op-1-instance-$(date +"%FT%H%M")

chmod -R a+rw result-float64-100000000-op-1-instance-$(date +"%FT%H%M")

docker run --rm -d --name smartwatts --net=host --privileged -v $(pwd)/result-float64-100000000-op-1-instance-$(date +"%FT%H%M"):/result-float64-100000000-op-1-instance-$(date +"%FT%H%M") powerapi/smartwatts-formula:latest --stream --input mongodb --model HWPCReport --uri mongodb://127.0.0.1 --db db_hwpc --collection report --output csv --model PowerReport --directory /result-float64-100000000-op-1-instance-$(date +"%FT%H%M") --cpu-base-freq 2200 --cpu-error-threshold 2.0 --disable-dram-formula --sensor-reports-frequency 1000



for i in {1..20}
do
	docker run --rm --name "float64-100000000-op-1-instance" ghcr.io/colinianking/stress-ng --cpu 1 --cpu-method float64 --cpu-ops 100000000 --times >> result-float64-100000000-op-1-instance-$(date +"%FT%H%M").txt
done
