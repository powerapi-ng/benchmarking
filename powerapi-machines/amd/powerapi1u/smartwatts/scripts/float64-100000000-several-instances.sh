...
do
        echo 'Loop started...'
        docker run --rm --name "float64-1000-op-several-instances" ghcr.io/colinianking/stress-ng --cpu 0 --cpu-method float64 --cpu-ops 100000000 --times >> result-float64-1000-op-several-instances-$(date +"%FT%H%M").txt
	echo 'Loop ended...'
done
